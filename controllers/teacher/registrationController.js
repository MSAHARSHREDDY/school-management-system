...
const teacherRegistration=async(req,res)=>{
    ...
    .custom((value)=>{
        return registrationService.find({email: value, deletedAt: null, }).then((emailInfo)=>{
    ...
    .custom((value)=>{
        return registrationService.find({mobile:value,deleteAt:null}).then((mobileInfo)=>{
...
for (let validation of validations) {
    ...
    if(errors.isEmpty())
    {
        var responseData=await registrationService.teacherRegistration(req.body)
